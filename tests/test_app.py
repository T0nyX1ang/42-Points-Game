...
class TestGameApp(unittest.TestCase):
    ...
    def test_game_process(self):
        ...
        self.assertRaises(ValueError, app.solve, '((((((((((((((1))))))))))))))')
        self.assertRaises(OverflowError, app.solve, '(((((((((((((((1)))))))))))))))')
        self.assertRaises(ValueError, app.solve, '1')
        self.assertRaises(SyntaxError, app.solve, '1+')
        self.assertRaises(SyntaxError, app.solve, '1+(')
        self.assertRaises(ValueError, app.solve, '42')
        self.assertRaises(ValueError, app.solve, '1+2+3+4+5+6+7+8+9-3')
        self.assertRaises(SyntaxError, app.solve, '-1+3')
        self.assertRaises(ValueError, app.solve, '1-(3-1)')
        self.assertRaises(SyntaxError, app.solve, '1**3')
        self.assertRaises(SyntaxError, app.solve,'1+3.0')
        self.assertRaises(ArithmeticError, app.solve,'1/0')
        self.assertRaises(ArithmeticError, app.solve, '3+4+6+7+12')
        self.assertRaises(ValueError, app.solve, '30+40+60+70+120')
        self.assertRaises(ValueError, app.solve, '300+400+600+700+1200')
        ...
        self.assertRaises(ValueError, app.solve, '11111111+222222+333333+555555')
    ...
    def test_rule_4(self):
        # additional tests for test rule 4
        app = FTPtsGame()
        app.generate_problem(problem=[3, 10, 6, 6, 13])
        app.start()
        app.solve('6*(6+(3+10)/13)')
        self.assertRaises(LookupError, app.solve, '6*(6+13/(3+10))')
        self.assertRaises(LookupError, app.solve, '6*(6+10/(13-3))')
        self.assertRaises(LookupError, app.solve, '6*(6+(13-3)/10)')
        self.assertRaises(LookupError, app.solve, '6*(6+3/(13-10))')
        self.assertRaises(LookupError, app.solve, '6*(6+(13-10)/3)')
        app.stop()
